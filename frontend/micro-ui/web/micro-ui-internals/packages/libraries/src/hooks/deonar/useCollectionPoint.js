...
const useCollectionPoint = ({ value }) => {
  ...
  const { mutateAsync: saveParkingDetails  } = useMutation(
    (data) => DeonarService.saveParkingDetail(data),
    {
      onSuccess: async () => {
        // Invalidate the specific query to refetch the data
        await queryClient.invalidateQueries({
          queryKey: ["fetchEntryFeeDetails"],
          exact: false, // Allow invalidating queries with partial matches (if necessary)
        });
      },
      onError: (error) => {
        console.error("Error saving Deonar details:", error);
