...
export const PostMortemInspectionModal = ({
  ...
  const handleChange = (field, value) => {
    ...
    if (field === "opinion" && value === "Partially Condemnation") {
      setShowAnimalQuarters(true);
    } else if (field === "opinion" && value !== "Partially Condemnation") {
      setShowAnimalQuarters(false);
    }
  };

  const handleSave = () => {
    if (!localData?.animalQuarters) {
      localData.animalQuarters = null;
    }
    handleUpdateValue(localData);
    toggleModal();
  };
