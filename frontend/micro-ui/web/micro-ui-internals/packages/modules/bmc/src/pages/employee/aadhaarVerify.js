...
const AadhaarVerifyPage = () => {
  const { t } = useTranslation();
  const history = useHistory();
  const actions = ["APPLY"];
  const availableActions = ["APPLY"];
  ...
  const handleVerifyAll = () => {
    ...
    getVerifyScheme.mutate(data, {
      onSuccess: () => {
        ...
        setTotalRecords((prevTotalRecords) => prevTotalRecords - selectedRows.length);
        setSelectedRows([]);
  ...
  return (
    <React.Fragment>
      <Title text={t(`BMC_Application_for_${t(selectedAction)}`)} />
              ...
              <div className="bmc-modal">
                <Modal onClose={closeModal} fullScreen hideSubmit={true} headerBarEnd={<CloseBtn onClick={closeModal} />}>
                  <p style={{ fontSize: "15px" }}>
                    <strong>{t("You are verifying all the Applications without Review!")}</strong>
                  </p>
                  <button
                    style={{
