...
public class PreApprovedVoucherAction extends GenericWorkFlowAction {
	 ...
	 * 
	 * @Autowired private PropertyTaxReceiptRegisterRepository
	 * propertyTaxReceiptRegisterRepository;
	 */
	//private PropertyTaxReceiptRegisterRepository ptrr= new PropertyTaxReceiptRegisterRepository();
	...
	public String loadvoucher() {
		...
		/*
		 * if (voucherHeader != null && voucherHeader.getState() != null) if
		 * (!validateOwner(voucherHeader.getState())) { final List<ValidationError>
		 * errors = new ArrayList<ValidationError>(); errors.add(new
		 * ValidationError("exp", "Invalid User" )); throw new
	...
	public String loadvoucherview() throws ApplicationException {
		...
		/*
		 * Long id = voucherHeader.getId(); System.out.println(id); String sqlQuery =
		 * "SELECT propertyid FROM property_tax_receipt_register WHERE voucherid = ?";
		 * Object ptt = entityManager.createNativeQuery(sqlQuery) .setParameter(1, id);
		 * // Set the parameter value for the voucherid .getSingleResult();
		 * 
		 * System.out.println("data: " + ptt);
		 * 
		 * String financialYear = "2023-24";
		 * 
		 * String jpql = "SELECT t1.id FROM FiscalPeriod t1 " +
		 * "INNER JOIN t1.financialYear t2 ON t1.financialYearId = t2.id " +
		 * "WHERE t2.financialYear = :financialYear";
		 * 
		 * javax.persistence.Query query = entityManager.createQuery(jpql);
		 * query.setParameter("financialYear", financialYear);
		 * 
		 * List<Integer> fiscalPeriodIds = query.getResultList();
		 */
		...
		// applicationIndex.setCityGrade(defaultString((String)
		// cityInfo.get(CITY_CORP_GRADE_KEY)));
		// applicationIndex.setDistrictName(defaultString((String)
		// cityInfo.get(CITY_DIST_NAME_KEY)));
		// applicationIndex.setRegionName(defaultString((String)
		// cityInfo.get(CITY_REGION_NAME_KEY)));
		return "view";
	}
		...
		try {
			...
			if (cutOffDate1 != null && voucherDate.compareTo(cutOffDate1) <= 0
				...
				if (voucherHeader.getVouchermis().getBudgetaryAppnumber() == null) {
					...
					addActionMessage(getText(egBillregister.getExpendituretype() + ".voucher.created",
							new String[] { voucherHeader.getVoucherNumber(), approverName }) + " And "
							+ getText("budget.recheck.sucessful",
									new String[] { voucherHeader.getVouchermis().getBudgetaryAppnumber() }));
			...
			if (((ValidationException) e).getErrors().get(0).getMessage() != null
				...
				throw new ValidationException(((ValidationException) e).getErrors().get(0).getMessage(),
						((ValidationException) e).getErrors().get(0).getMessage());
			else
			 ...
			 * ValidationException s = (ValidationException) e; final List<ValidationError>
			 * errors = new ArrayList<ValidationError>(); errors.add(new
			 * ValidationError("exp", s.getErrors().get(0).getMessage())); throw new
			 * ValidationException(errors); } LOGGER.error(e.getMessage()); final
			 * List<ValidationError> errors = new ArrayList<ValidationError>(); final
			 * ValidationException s = (ValidationException) e; errors.add(new
			 * ValidationError("exception", s.getErrors().get(0).getMessage())); throw new
			 * ValidationException(errors); }
			 */
		getHeaderMandateFields();
	...
	public String update() {
		...
		try {
			...
			if (null == type)
				type = "default";
			EmployeeInfo employee = microserviceUtils
					.getEmployeeByPositionId(voucherHeader.getState().getOwnerPosition());
			...
			if (voucherHeader.getId() != null && voucherHeader.getId().compareTo(new Long(0)) > 0) {
				finDashboardService.publishEvent(FinanceEventType.voucherUpdateById,
						new HashSet<>(Arrays.asList(voucherHeader.getId())));
			}
		} catch (final ValidationException | ApplicationException e) {
	...
	public void validate() {
		 ...
		 * parameters.get("comments")[0].length()>1024) addFieldError("comments",
		 * "Max. length exceeded");
		 */
	}
	...
	public String getMasterName(final String name) {
	    ...
	    if (voucherHeader != null) {
	        if ("fund".equals(name) && voucherHeader.getFundId() != null) {
	            val = voucherHeader.getFundId().getName();
	        } else if ("fundsource".equals(name) && voucherHeader.getVouchermis().getFundsource() != null) {
	            val = voucherHeader.getVouchermis().getFundsource().getName();
	        } else if ("department".equals(name) && voucherHeader.getVouchermis().getDepartmentcode() != null
	            ...
	            val = list != null ? list.getName() : "";
	        } else if ("scheme".equals(name) && voucherHeader.getVouchermis().getSchemeid() != null) {
	            val = voucherHeader.getVouchermis().getSchemeid().getName();
	        } else if ("subscheme".equals(name) && voucherHeader.getVouchermis().getSubschemeid() != null) {
	            val = voucherHeader.getVouchermis().getSubschemeid().getName();
	        } else if ("field".equals(name) && voucherHeader.getVouchermis().getDivisionid() != null) {
	            val = voucherHeader.getVouchermis().getDivisionid().getName();
	        } else if ("functionary".equals(name) && voucherHeader.getVouchermis().getFunctionary() != null) {
	            ...
	            Long id = voucherHeader.getId();
	            String sqlQuery = "SELECT COALESCE(ptdr.propertyid, ptrr.propertyid) AS propertyid " +
	                    "FROM voucherheader v " +
	                    "LEFT JOIN property_tax_demand_register ptdr ON v.id = ptdr.voucherid " +
	                    "LEFT JOIN property_tax_receipt_register ptrr ON v.id = ptrr.voucherid " +
	                    "WHERE v.id = ?";
	            Object ptt = entityManager.createNativeQuery(sqlQuery)
	                                     .setParameter(1, id)
	                                     .getSingleResult();
	            val = (ptt != null) ? String.valueOf(ptt) : "";
	        } else if ("budgetaryAppnumber".equals(name)) {
	            val = voucherHeader.getVouchermis().getBudgetaryAppnumber();
	        }
	    } else {
	        // Handle cases for egBillregister
	    }
	...
	public void getMasterDataForBillVoucher() throws ApplicationException {
		...
		if (voucherHeader != null) {
			...
			/*
			 * // this is for only vouchers, which do not have the bills List<VoucherDetail>
			 * voucherDetailList=new ArrayList<VoucherDetail>(); voucherDetailList =
	...
	protected Boolean isDefaultAutoPopulateCurrDateEnable() {
		List<AppConfigValues> appConfigByModuleNameAndKeyName = appConfigValuesService
				.getConfigValuesByModuleAndKey("EGF", "DEFAULT_AUTO_POPULATE_CURR_DATE");
		return !appConfigByModuleNameAndKeyName.isEmpty()
				? "Y".equalsIgnoreCase(appConfigByModuleNameAndKeyName.get(0).getValue())
				: true;
	}
	...
	/*
	 * public String uniqueptid(String voucherid) { Optional<Long>
	 * propertyIdOptional =
	 * propertyTaxReceiptRegisterRepository.findPropertyIdByVoucherId(voucherId);
	 * String pttString = null; // Declare pttString outside of the if block and
	 * initialize it to null
	 * 
	 * if (propertyIdOptional.isPresent()) { String sqlQuery =
	 * "SELECT propertyid FROM property_tax_receipt_register WHERE voucherid = ?";
	 * Object ptt = entityManager.createNativeQuery(sqlQuery) .setParameter(1,
	 * voucherId) // Set the parameter value for the voucherid .getSingleResult();
	 * 
	 * System.out.println("data: " + ptt);
	 * 
	 * pttString = (ptt != null) ? String.valueOf(ptt) : null;
	 * System.out.println("data: " + pttString); // Do something with the propertyId
	 * } else { // Handle case when no property id is found for the given voucher id
	 * } return pttString; }
	 */
